...
class BerzerkBacktester:
            ...
            try:
                ...
                if action in ['ACHETER', 'ACHAT', 'BUY']:
                    ...
                    if ticker:  # Seulement si on a un ticker valide
                        ...
                        print(f"✅ Décision d'ACHAT trouvée: {ticker} ({title[:30]}...)")
                    else:
    ...
    def simulate_trade(self, trade: Dict) -> Optional[Dict]:
        ...
        print(f"📊 Simulation du trade {ticker} (décision du {decision_date.strftime('%Y-%m-%d')})")
        ...
        try:
            ...
            start_date = buy_date - timedelta(days=10)  # Plus de marge pour les dates récentes
            end_date = datetime.now() + timedelta(days=2)  # Utiliser date actuelle + marge
            
            print(f"   📅 Période de données: {start_date.strftime('%Y-%m-%d')} → {end_date.strftime('%Y-%m-%d')}")
            ...
            for date, row in hist.iterrows():
                if date.date() >= buy_date.date():
                    ...
                    print(f"   💰 Prix d'achat trouvé: {buy_price:.2f} USD le {date.strftime('%Y-%m-%d')}")
                    break
            ...
            if buy_price is None:
                if not hist.empty:
                    ...
                    print(f"   💰 Prix d'achat (dernier disponible): {buy_price:.2f} USD le {actual_buy_date.strftime('%Y-%m-%d')}")
                else:
            ...
            if datetime.now().date() < sell_date.date():
                ...
                try:
                    ...
                    print(f"   📈 Prix de vente (actuel): {sell_price:.2f} USD le {actual_sell_date.strftime('%Y-%m-%d')}")
                except:
                    # Si pas d'info actuelle, utiliser le dernier prix historique
                    sell_price = hist.iloc[-1]['Close']
                    actual_sell_date = hist.index[-1]
                    print(f"   📈 Prix de vente (dernier historique): {sell_price:.2f} USD le {actual_sell_date.strftime('%Y-%m-%d')}")
            else:
                ...
                for date, row in hist.iterrows():
                    if date.date() >= sell_date.date():
                        ...
                        print(f"   📈 Prix de vente trouvé: {sell_price:.2f} USD le {date.strftime('%Y-%m-%d')}")
                        break
                ...
                if sell_price is None:
                    sell_price = hist.iloc[-1]['Close']
                    actual_sell_date = hist.index[-1]
                    print(f"   📈 Prix de vente (dernier disponible): {sell_price:.2f} USD le {actual_sell_date.strftime('%Y-%m-%d')}")
            ...
            partial_note = " (⚠️ Simulation partielle)" if result['is_partial_simulation'] else ""
    ...
    def display_results(self, results: Dict):
        ...
        if results['win_rate'] > 60:
            print("🔥 Performance EXCELLENTE ! Le système montre une très bonne capacité prédictive.")
